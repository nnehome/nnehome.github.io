...
3. Repeat step 1-3 for :math:`\ell=1,...,L^{*}` to construct the set :math:`\{\boldsymbol{m}^{(\ell)},\boldsymbol{\theta}^{(\ell)}\}_{\ell=1}^{L^{*}}`. We can also create a validation set by repeating these steps more times for :math:`\ell=L+1,...,L^{*}`
